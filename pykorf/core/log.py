...
Example:
    >>> from pykorf.core.log import get_logger, bind_context
    >>> logger = get_logger()
...
class FlashLogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        # Only capture WARNING and above — INFO logs are internal operational
        # noise that should not be shown to the user.
        ...
        # Use only the human-readable message, no structured key=value pairs
        msg = record.getMessage()
        ...
        alert_type = level_map.get(record.levelno, "warning")
...
def flash_logs():
    ...
    Example:
        >>> from pykorf.core.log import get_logger, flash_logs
