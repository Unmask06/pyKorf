...
from pykorf.core.types import ExportOptions
...
class IOService:
    ...
    Example:
        ...
        >>> model._io_service.save()  # Delegates to IOService
        >>> dfs = model._io_service.to_dataframes()
    """
    ...
    def save(self, path: str | Path | None = None, *, overwrite: bool = True) -> None:
        ...
        Example:
            >>> model._io_service.save()  # Overwrite original
            >>> model._io_service.save("new_file.kdf")  # Save as new file
        """
    ...
    def to_excel(self, path: str | Path, *, overwrite: bool = True) -> None:
        ...
        Example:
            >>> model._io_service.to_excel("Pumpcases.xlsx")
        """
        dest_path = Path(path)
        if not overwrite and dest_path.exists():
            raise ExportError(
    ...
    def _model_to_dataframes(self) -> dict[str, pd.DataFrame]:
        ...
        from pykorf.core.utils import format_line
    ...
    def model_from_dataframes(cls, dfs: dict[str, pd.DataFrame]) -> Model:
        ...
        Example:
            ...
            ```
        """
        from pykorf.core.model import Model as _Model
