...
if HAS_PYDANTIC:
    class FluidProperties(KdfBaseModel):
        ...
        liquid_viscosity_cp: list[float] | None = Field(
            None, description="Liquid viscosity in cP"
        )
        liquid_surface_tension: list[float] | None = Field(
            None, description="Liquid surface tension in dyne/cm"
        )
        vapor_density_kg_m3: list[float] | None = Field(
            None, description="Vapor density in kg/m³"
        )
        vapor_viscosity_cp: list[float] | None = Field(
            None, description="Vapor viscosity in cP"
        )
